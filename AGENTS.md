...
It implements AI-powered features ranging from simple LLM chat and streaming to multi-agent pipelines, live database schema inspection, web research integration, and an autonomous state-machine optimization engine.
...
└── com/aivashin/
    ...
    │   │   ├── DatabaseDependencies.kt         # Database connection pool & ChatHistoryProvider bindings
    │   │   ├── ToolsDependencies.kt            # ToolRegistry initialization & tool bindings
    ...
    │   └── models.kt                           # Request/Response HTTP DTOs (ChatRequest, ChatResponse)
    ├── service/
    │   ├── agent/
    ...
    └── tool/
        └── DatabaseTools.kt                    # JDBC introspection tools (GetTableSchemaTool, ListDatabaseTablesTool)
```
...
- **Database & Pooling**: PostgreSQL (`org.postgresql:postgresql`), HikariCP (`com.zaxxer:HikariCP`), H2 (for tests).
...
### C. Graph State-Machine Pipeline (`DatabaseOptimizerGraphService`)
...
3. `contextAggregatorNode`: Concurrently fetches live JDBC table schemas and executes Tavily web searches for PostgreSQL best practices.
...
1. **Dependency Injection**: Always use Ktor DI (`io.ktor.server.plugins.di.*`).
   ...
   - In service constructors: annotate property injections with `@Property("config.path")` or `@Named("beanName")`.
...
4. **Error Handling**: Collect system errors into `ctx.systemErrors` or `validationErrors` instead of throwing unhandled exceptions inside graph nodes.
5. **No Hardcoded Secrets**: Never check in API keys or passwords. Always reference configuration via `application.yaml` placeholders (`$GEMINI_API_KEY`).
6. **Preserve Code Structure**: Keep imports, package structures, and documentation intact.
...
2. **Web Endpoint Tests (`com.aivashin.web.RoutesWebTest`)**:
   ...
   - Runs using Ktor's `testApplication` engine loaded with `application-test.yaml`.
   - **No active DB or Docker container required** — `GoogleLLMClient` is mocked via constructor mocking (`mockkConstructor`).
